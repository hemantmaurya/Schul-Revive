...
import { Plus, Edit2, Trash2, Upload, Search, Loader2, ChevronLeft, ChevronRight } from 'lucide-react';
    ...
    return (
        <div className="space-y-6">
            <div className="flex justify-between items-center">
                <h1 className="text-3xl font-bold text-gray-800">Test CRUD Demo</h1>
                <button
                    onClick={() => openModal()}
            ...
            <div className="bg-white rounded-2xl shadow-sm overflow-hidden">
                <table className="w-full">
                    ...
                    <tbody>
                    {loading ? (
                        <tr><td colSpan="6" className="py-12 text-center"><Loader2 className="animate-spin mx-auto" size={40} /></td></tr>
                    ) : tests.length === 0 ? (
                        ...
                        tests.map(test => (
                            <tr key={test.id} className="border-b hover:bg-gray-50">
                                ...
                                <td className="px-6 py-4 text-gray-600 max-w-xs truncate">{test.description}</td>
                                <td className="px-6 py-4">
                    <span className={`px-3 py-1 rounded-full text-sm ${test.status === 'Active' ? 'bg-green-100 text-green-700' : 'bg-gray-100 text-gray-600'}`}>
                      {test.status}
                    </span>
                                </td>
                                <td className="px-6 py-4 font-medium">{test.priority}</td>
            ...
            <div className="flex items-center justify-between bg-white p-4 rounded-2xl shadow-sm">
                <div className="text-sm text-gray-600">
                    Showing <span className="font-medium">{tests.length}</span> of {pagination.total} records
                </div>
                ...
                <div className="flex items-center gap-3">
                    <button
                        ...
                        disabled={page === 1}
                        className="flex items-center gap-1 px-4 py-2 border rounded-xl disabled:opacity-50 hover:bg-gray-50"
                    >
                ...
                <form onSubmit={handleSubmit} className="space-y-5">
                    ...
                    <div className="flex justify-center">
                        <div className="relative">
                            <img src={preview || "https://via.placeholder.com/150"} className="w-32 h-32 object-cover rounded-2xl border" />
                    ...
                    <button type="submit" disabled={submitting} className="w-full bg-blue-600 text-white py-4 rounded-xl font-semibold hover:bg-blue-700 disabled:opacity-70">
